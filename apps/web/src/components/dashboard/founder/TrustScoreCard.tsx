...
import type { TrustScore } from "@vittamhub/types";
...
/** Trust Score is always computed server-side (TrustScoreService) — this card only renders it, never assigns it. */
    ...
    <Card className="flex flex-col gap-4">
      <CardHeader className="flex-row items-center justify-between pb-0">
        <CardTitle className="flex items-center gap-2 text-base">
          <ShieldCheck className="h-4 w-4 text-brand-primary" /> Trust Score
        </CardTitle>
        <Badge variant={BAND_BADGE_VARIANT[trustScore.band] ?? "neutral"}>{trustScore.band}</Badge>
      </CardHeader>

      <div className="flex items-center gap-4">
        <span className={`font-numeric text-4xl font-bold ${scoreColor(trustScore.score)}`}>{trustScore.score}</span>
        <span className="text-sm text-text-secondary">out of 100, based on {trustScore.factors.length} signals</span>
      </div>

      <button type="button" onClick={() => setExpanded((v) => !v)} className="text-left text-xs font-medium text-brand-primary hover:underline">
        {expanded ? "Hide details" : "See what makes up your score"}
      </button>
